define([], function() {
    ...
    return async function({ apis, $router, $account, $channel, $unique, $scope, $model, $getUser, $sendMessage, $apply }) {
        ...
        apis.addSiteNumberToAccount = function addSiteNumberToAccount(el) {
            ...
            $(el[2]).empty()
                .append(apis.createElement(account, account, "account"))
                .append(apis.createElement(channel, unique, "channel"))

        };

        $scope.start = function() {
            $("#divCookie>ul:not(.TrHead):not(.TrHead2)")
                .filter((i, { firstElementChild, childElementCount }) => {
                    return firstElementChild.outerText && childElementCount > 5
                }).each((i, { children }) => {
        ...
        $scope.getUserRegions = function getUserRegions() {
