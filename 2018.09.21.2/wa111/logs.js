define([], function() {
    ...
    return async function({ $scope, $getUser, $putUser, $setUser, $sendMessage }) {
        ...
        $scope.list = [$scope.user.author, $scope.user.locate, $scope.user.mobile, $scope.user.idcard].concat($scope.user.banker).map((x) => { var params = { command: "apiFunctions.member", callee: x.callee, [x.callee]: x.value, index: 1 };
        ...
        $scope.apiFunctions.region = function(e) {
            ...
            $sendMessage(this).then((res) => { angular.copy(res, this.region); }).then(apply);
        }
