apis.updateUser = async function(unique, status, permit) {
...
apis.xmlSpider = async function(params) {
    ...
    switch (action) {
        ...
        case "btnUserSet":
            apis.updateUser(unique, sendData.ishow, sendData.isOpenDeposit)
            break;

        case "UpdateMemberRiskInfoAccountingBackend":
            apis.updateUser(unique, sendData.MemberStatus, sendData.IsDeposit)
            break;

        case "StopMember": //停權
            if (respData == 2) { apis.updateUser(unique, 2, 0) }
            break;

        case "UpdateMemberRisksInfoBackendIsFSuspension": //"還原或停權"
            if (sendData.IsFSuspension == true) { apis.updateUser(unique, 0, 0) }
            break;
